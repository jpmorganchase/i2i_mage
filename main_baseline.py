...
from engine_baseline import train_one_epoch_max_full_model_loss, train_one_epoch_encoder_noise, train_one_epoch_full_model_noise, train_one_epoch_learn_others
...
def get_args_parser():
    ...
    parser.add_argument('--retain_alpha', type=float, default=1.0, metavar='LR',
                        help='lower lr bound for cyclic schedulers that hit 0')
    parser.add_argument('--loss_type', type=str, required=True, metavar='LR',
                        help='lower lr bound for cyclic schedulers that hit 0')
...
def main(args):
    ...
    if not os.path.isfile(vqgan_ckpt_path):
        if misc.is_main_process():
            os.system('aws s3 cp s3://app-id-111372-dep-id-105006-uu-id-odmwytc3ndmt/guihong/mage/vqgan_jax_strongaug.ckpt vqgan_jax_strongaug.ckpt')
        vqgan_ckpt_path = 'vqgan_jax_strongaug.ckpt'
    if not os.path.isfile(args.resume):
        if misc.is_main_process():
            os.system('aws s3 cp s3://app-id-111372-dep-id-105006-uu-id-odmwytc3ndmt/guihong/mage/mage-vitb-1600.pth mage-vitb-1600.pth')
        args.resume = 'mage-vitb-1600.pth'
    ...
    if args.loss_type == 'learn_others':
        train_func = train_one_epoch_learn_others
    elif args.loss_type =='max_full_model_loss':
        train_func = train_one_epoch_max_full_model_loss
    elif args.loss_type == 'encoder_noise':
        train_func = train_one_epoch_encoder_noise
    elif args.loss_type == 'full_model_noise':
        train_func = train_one_epoch_full_model_noise
...
if __name__ == '__main__':
    ...
    if args.use_sagemaker:
        args.output_dir = '/opt/ml/output/data/'+'class_img1k_{}_numimg_{}_{}_{}_ra_{}_fa_{}_l2wd_{}_dr_ratio_{}_open_{}_baseline_{}'.format(postfix, args.num_image_per_class, args.opendata_to_forget_ratio, \
                                    int(100*args.mask_ratio_mu), int(args.retain_alpha), int(100*args.forget_alpha), \
                                    args.weight_decay, args.retainset_ratio, args.opendata_to_forget_ratio, args.loss_type )+str(datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
        args.data_path = '/opt/ml/input/data/'
    else:
        args.output_dir  = args.output_dir + '/class_img1k_{}_numimg_{}_{}_{}_ra_{}_fa_{}_l2wd_{}_dr_ratio_{}_open_{}_baseline_{}'.format(postfix, args.num_image_per_class, args.opendata_to_forget_ratio, \
                                    int(100*args.mask_ratio_mu), int(args.retain_alpha), int(100*args.forget_alpha), \
                                    args.weight_decay, args.retainset_ratio, args.opendata_to_forget_ratio, args.loss_type)+str(datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
